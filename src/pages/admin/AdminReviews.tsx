...
export default function AdminReviews() {
  ...
  function Stars({ rating }: { rating: number }) {
    return (
      <span>
        {[1, 2, 3, 4, 5].map(i => (
          <span key={i} style={{ color: i <= rating ? 'var(--color-gold-primary)' : '#52367c' }}>★</span>
        ))}
      </span>
    );
  }
  ...
  return (
    <DashboardLayout>
      <h1 className="font-display text-3xl font-bold mb-6 flex items-center gap-3" style={{ color: 'var(--color-text-primary)' }}><IcoStar size={25} color={roleColor('admin')} /> Gestion des avis</h1>

      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="card-glass p-4 text-center">
          <div className="text-2xl font-bold" style={{ color: 'var(--color-gold-primary)' }}>{reviews.length}</div>
          <div className="text-xs mt-1" style={{ color: 'var(--color-text-secondary)' }}>Total avis</div>
        </div>
        <div className="card-glass p-4 text-center">
          <div className="text-2xl font-bold" style={{ color: '#10b981' }}>{avgRating}</div>
          <div className="text-xs mt-1" style={{ color: 'var(--color-text-secondary)' }}>Note moyenne</div>
        </div>
        <div className="card-glass p-4 text-center">
          <div className="text-2xl font-bold" style={{ color: '#ef4444' }}>
            {reviews.filter(r => r.rating <= 2).length}
          </div>
          <div className="text-xs mt-1" style={{ color: 'var(--color-text-secondary)' }}>Avis négatifs</div>
        </div>
      </div>
      ...
      <div className="card-glass p-4 mb-6 flex flex-wrap gap-3">
        ...
        <span className="text-sm my-auto" style={{ color: 'var(--color-text-secondary)' }}>{filtered.length} avis</span>
      </div>

      {loading ? (
        <div className="text-center py-16" style={{ color: 'var(--color-text-secondary)' }}>Chargement...</div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16" style={{ color: 'var(--color-text-secondary)' }}>Aucun avis</div>
      ) : (
                ...
                <div className="flex items-start gap-3 flex-1">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 flex-wrap mb-1">
                      <span className="font-medium text-sm" style={{ color: 'var(--color-text-primary)' }}>{r.reviewer?.full_name || '–'}</span>
                      <span className="text-xs" style={{ color: 'var(--color-text-muted)' }}>→ {r.reviewed?.full_name || '–'}</span>
